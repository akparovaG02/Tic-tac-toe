...
public class TiltokToeBoard extends View {
    private final int boardColor, XColor, OColor, winningLineColor;
    private boolean winningLine = false;
    private final Paint paint = new Paint();
    private final GameLogic game;
    private int cellSize = getWidth()/3;
    public TiltokToeBoard(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.TiltokToeBoard, 0,0);

        game = new GameLogic();
        try {
            boardColor = a.getInteger(R.styleable.TiltokToeBoard_boardColor,0);
            XColor = a.getInteger(R.styleable.TiltokToeBoard_XColor,0);
            OColor = a.getInteger(R.styleable.TiltokToeBoard_OColor,0);
            winningLineColor = a.getInteger(R.styleable.TiltokToeBoard_winningLineColor,0);
        } finally {
            a.recycle();
        }
    }
